...
class YCompressTiltFieldGenerator(Node):
    def __init__(self):
        super().__init__('y_compress_tilt_field_generator')
        self.width = 25.0
        self.height = 25.0
        self.resolution = 1.0
        self.center_x = 12.5
        ...
        self.marker_pub = self.create_publisher(Marker, '/gaussian_field/y_compress_tilt', qos_viz)
        self.temp_pub = self.create_publisher(Float32, '/gaussian_field/y_compress_tilt/temperature', 10)
        self.temp_noisy_pub = self.create_publisher(Float32, '/gaussian_field/y_compress_tilt/temperature_noisy', 10)
        ...
        self.get_logger().info("Y-compress-tilt field initialized")
    def generate_field(self):
        ...
        self.get_logger().info(f"Y-compress-tilt field: [{self.temp_min:.2f}, {self.temp_max:.2f}]°C")
    ...
    def publish_visualization(self):
        ...
        marker.ns = "y_compress_tilt_field"
        marker.id = 0
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                p = Point()
                p.x, p.y, p.z = float(self.X[i, j]), float(self.Y[i, j]), -0.5
...
def main(args=None):
    ...
    node = YCompressTiltFieldGenerator()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
