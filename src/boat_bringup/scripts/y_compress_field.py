...
class YCompressFieldGenerator(Node):
    def __init__(self):
        super().__init__('y_compress_field_generator')
        self.width = 25.0
        self.height = 25.0
        self.resolution = 1.0
        self.center_x = 12.5
        ...
        self.marker_pub = self.create_publisher(Marker, '/gaussian_field/y_compress', qos_viz)
        self.temp_pub = self.create_publisher(Float32, '/gaussian_field/y_compress/temperature', 10)
        self.temp_noisy_pub = self.create_publisher(Float32, '/gaussian_field/y_compress/temperature_noisy', 10)
        ...
        self.get_logger().info("Y-compress field initialized")
    def generate_field(self):
        ...
        self.get_logger().info(f"Y-compress field: [{self.temp_min:.2f}, {self.temp_max:.2f}]°C")
    ...
    def publish_visualization(self):
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                p = Point()
                p.x, p.y, p.z = float(self.X[i, j]), float(self.Y[i, j]), -0.4
                marker.points.append(p)
                normalized = (temp - self.temp_min) / (self.temp_max - self.temp_min)
                color = ColorRGBA()
                color.a = 0.8
                color.r = normalized * 0.3
                color.g = 0.5 + normalized * 0.5
                color.b = 1.0 - normalized
                marker.colors.append(color)
        self.marker_pub.publish(marker)
def main(args=None):
    rclpy.init(args=args)
    node = YCompressFieldGenerator()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
