#!/usr/bin/env python3
"""
Exact (stationary RBF) planner over MAVROS.
...
Usage:
  ros2 launch boat_bringup exact.launch.py
  ros2 launch boat_bringup exact.launch.py field_type:=x_compress trial:=1
  ros2 launch boat_bringup exact.launch.py fcu_url:=udp://:14540@127.0.0.1:14557
...
def generate_launch_description():
    ...
    # Wrap mavros's canonical PX4 launch file — pulls in px4_pluginlists.yaml
    # (plugin_denylist) and px4_config.yaml (per-plugin params) so plugin
    # initialization order doesn't blow up.
    ...
    # Static TF: world <-> map (identity). MAVROS publishes its topics in `map`;
    # without this, RViz can't transform them into the `world` fixed frame.
